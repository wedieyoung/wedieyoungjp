...
各ニュースには**記事本文（アーティスト紹介等）**を付けられます。`article` がある場合、
一覧の「READ MORE →」がモーダル表示に変わります:
...
```js
{
  ...
  title: "タイトル",
  body: "一覧に表示される短い要約",
  link: "",                        // articleがある場合は""推奨（リンクはarticleLinksへ）
  image: "https://...（ジャケット等。モーダル冒頭に表示）",
  article: [                       // 記事本文（段落ごとに1つ）
    "段落1…",
    "【プロフィール】アーティスト名",  // 【】で始まる段落は見出しとして表示
    "プロフィール文…",
    "行1\n行2"                     // \n で段落内改行（トラックリスト等に）
  ],
  gallery: [ "https://...photo.jpg" ],  // 写真（クリックで拡大。省略可）
  articleLinks: [                  // モーダル下部のリンクボタン
    { label: "LISTEN / STREAM", url: "https://..." },
    { label: "アーティスト (X)", url: "https://..." }
  ]
},
```
...
ギャラリーの写真は**クリックで拡大表示**されます（←→キー・矢印ボタンで前後の写真へ移動、
Escまたは✕で閉じる）。一覧には軽量な1024px画像を使い、拡大時のみ高解像度版
（ファイル名の `-1024x576` 等を `-scaled` に置き換えたURL）を自動で読み込みます。
高解像度版が存在しない場合はサムネイルをそのまま表示するので、URLはどちらでも安全です。
...
- **全リリースにBandcampボタンを追加** — SubmitHubの各作品ページからBandcampリンクを
  ...
  全39作品中38作品にBandcampリンクが付いた状態に
  （Producer Pack vol.1 のみGumroad販売でBandcamp未存在のため対象外）

### 2026-06-11（その3）
- **リリース記事の本文を全て移植** — 旧サイトのRELEASE/GOODSニュース15記事の本文
  （アーティスト紹介・BIOGRAPHY・アルバム解説・トラックリスト・販売情報、日英両方）を
  `data.js` の `article` フィールドに反映。ニュース一覧の「READ MORE →」からモーダルで
  全文＋ジャケット画像＋関連リンク（配信/アーティストSNS/ストア）を表示
- vol.7 Tee の商品写真8枚もギャラリー表示（クリックで拡大）
- ※旧サイト記事の日付誤記（digital[F10W] の「2025年1月9日」）は「2026年1月9日」に修正

### 2026-06-11（その2）
- **写真ライトボックス機能を追加** — イベントレポートのギャラリー写真をクリックすると
  全画面で拡大表示。矢印ボタン・←→キーで前後の写真へ移動、Esc/✕/背景クリックで閉じる
- 一覧表示は軽量な1024px画像のまま、拡大時のみ高解像度版（2560px・`-scaled`版）を
  読み込む方式。高解像度版が無い写真は自動でサムネイルにフォールバック
